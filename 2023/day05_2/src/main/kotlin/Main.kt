...
fun main(args: Array<String>) {
    println("AOC 2023, Day 5, Part 2 starting!!!")
    ...
    for(seed in allSeeds.seeds) {
        for(s in seed.start..<seed.start + seed.end) {
            val answer = calculateAnswer(s)
    ...
    println("AOC 2023, Day 5, Part 2 completed!!!")
}
...
data class DupleLong (val start:Long, val end:Long)

class Seeds {
    val seeds:HashSet<DupleLong> = HashSet()
}
...
fun parseLine(line:String) {
    ...
    if((lastMatch.compareTo(PlantingOptions.SEEDS.theName) == 0) || (line.substring(0, 6).compareTo(PlantingOptions.SEEDS.theName) == 0)) {
        ...
        for(c in line.split(' ')) {
            ...
            if(seedPosition == 0L) {
                seedPosition = c.toLong()
                // continue to read range value
                continue
            }

            seedRange = c.toLong()

            allSeeds.seeds.add(DupleLong(seedPosition, seedRange))

//            for(seed in seedPosition ..< (seedPosition + seedRange)) {
//                allSeeds.seeds.add(seed)
//            }
    ...
    if((lastMatch.compareTo(PlantingOptions.SOIL_TO_FERTILIZER_MAP.theName) == 0) || (line.compareTo(PlantingOptions.SOIL_TO_FERTILIZER_MAP.theName) == 0)) {
    ...
    } else if ((lastMatch.compareTo(PlantingOptions.WATER_TO_LIGHT_MAP.theName) == 0) || (line.compareTo(PlantingOptions.WATER_TO_LIGHT_MAP.theName) == 0)) {
        if(lastMatch.isEmpty()) {
        ...
        return
    } else if((lastMatch.compareTo(PlantingOptions.LIGHT_TO_TEMPERATURE_MAP.theName) == 0) || (line.compareTo(PlantingOptions.LIGHT_TO_TEMPERATURE_MAP.theName) == 0)) {
        if(lastMatch.isEmpty()) {
        ...
        return
    } else if((lastMatch.compareTo(PlantingOptions.TEMPERATURE_TO_HUMIDITY_MAP.theName) == 0) || (line.compareTo(PlantingOptions.TEMPERATURE_TO_HUMIDITY_MAP.theName) == 0)) {
        if(lastMatch.isEmpty()) {
        ...
        return
    } else if((lastMatch.compareTo(PlantingOptions.HUMIDITY_TO_LOCATION_MAP.theName) == 0) || (line.compareTo(PlantingOptions.HUMIDITY_TO_LOCATION_MAP.theName) == 0)) {
        if(lastMatch.isEmpty()) {
