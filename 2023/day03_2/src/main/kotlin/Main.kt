...
fun main(args: Array<String>) {
    println("AOC 2023, Day 3, Part 2 starting!!!")
    ...
    println("AOC 2023, Day 3, Part 2 completed!!")
}

class Schematic {
    private val lines:ArrayList<String> = ArrayList()
    private var gearRatios:ArrayList<Int> = ArrayList()
    ...
    private fun scanForParts(x:Int, y:Int) {
        ...
        if(attachedParts.size > 1) {
            var gearRatio = 1
            attachedParts.stream().forEach { gearRatio *= it }
    ...
    fun getAttachedParts():Int {
        var answer = 0
        gearRatios.stream().forEach { answer += it }
        return answer
    }
}