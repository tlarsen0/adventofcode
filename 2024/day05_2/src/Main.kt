...
fun main(args: Array<String>) {
    println("AOC 2024, Day 5, Part 2 starting!!!!")
    ...
    for(pageRun in pageRunList) {
        if (!pageRun.checkPages(pageOrderRuleList)) {
            val fixedPageRun = pageRun.fixPages(pageOrderRuleList)
            val middleIndex = fixedPageRun.pageList.size / 2
            middlePageTotal += fixedPageRun.pageList[middleIndex]
            println("Corrected Pages ${fixedPageRun.pageList} is now valid, middle page is ${fixedPageRun.pageList[middleIndex]}")
        }
    }

    println("Middle Page Total: $middlePageTotal")

    println("AOC 2024, Day 5, Part 2 completed!!!")
}
...
class PageRun {
    ...
    fun fixPages(pageOrderRule: ArrayList<PageOrderRule>): PageRun {
        val fixedPageRun = PageRun()
        fixedPageRun.pageList.addAll(pageList)
        ...
        var i = 0
        outer@ while(i < fixedPageRun.pageList.size - 1) {
            val pageFirst = fixedPageRun.pageList[i]
            val pageSecond = fixedPageRun.pageList[i + 1]
            for(rule in combinedBeforeRules) {
                if (rule.pageBefore == pageSecond && rule.pageAfter == pageFirst) {
                    // swap pageFirst and pageSecond
                    fixedPageRun.pageList[i + 1] = pageFirst
                    fixedPageRun.pageList[i] = pageSecond
